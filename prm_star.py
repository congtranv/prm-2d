...
def sample_points(sx, sy, gx, gy, rr, ox, oy, obstacle_kd_tree, n):
    N_SAMPLE = n;
    max_x = max(ox)
    max_y = max(oy)
    min_x = min(ox)
    ...
    while len(sample_x) <= N_SAMPLE*3:
        ...
        if itr==0:
            itr = 2
        tx = math.pow(math.log(itr)/itr,(1/10))
        ty = math.pow(math.log(itr)/itr,(1/10))
        tx = (random.random() * (max_x - tx)) + tx
        ty = (random.random() * (max_y - ty)) + ty
        #print(str(tx) + " " + str(ty) + "\n")
